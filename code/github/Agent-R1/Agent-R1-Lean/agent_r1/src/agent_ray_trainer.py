...
from agent_r1.src.test_utils import TestUtil
...
def apply_kl_penalty(data: DataProto, kl_ctrl: core_algos.AdaptiveKLController, kl_penalty='kl'):
    ...
    # print('[TOKEN LEVEL REWARD]', token_level_rewards[0], beta, kld[0])
    # print('[ACTION MASK]', action_mask[0].tolist())
    valid_mask = action_mask.sum(dim=-1) > 0
    invalid_mask = action_mask.sum(dim=-1) < 1
    print(invalid_mask)
    current_kl = masked_mean(kld, mask=action_mask, axis=-1)  # average over sequence
    current_kl = current_kl[valid_mask]
    if current_kl.numel() > 0:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer('step', timing_raw):
                    ...
                    with _timer('adv', timing_raw):
                        ...
                        # TestUtil.print_masked_batch(batch, self.tokenizer)
