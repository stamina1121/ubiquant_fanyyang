...
def compute_policy_loss(old_log_prob, log_prob, advantages, action_mask, cliprange):
    ...
    """
    print('[ACTION_MASK_IS_ZERO]', action_mask.sum(axis=None) < 1.0)
    if action_mask.sum(axis=None) < 1.0:
        return 0.0, 0.0, 0.0
    negative_approx_kl = log_prob - old_log_prob
    ratio = torch.exp(negative_approx_kl)
    # print('[NEGATIVE_APPROX_KL]', negative_approx_kl)
    # print('[RATIO]', ratio)
...
def compute_entropy_loss(logits, action_mask):
    ...
    """
    if torch.abs(action_mask.sum(axis=None)) < 1.0:
        return 0.0
    # compute entropy
...
def compute_value_loss(vpreds, returns, values, state_mask, cliprange_value):
    ...
    """
    if torch.abs(state_mask.sum(axis=None)) < 1.0:
        return 0.0, 0.0
